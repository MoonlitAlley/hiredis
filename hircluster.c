...
int redisClusterSetOptionAuthPassword(redisClusterContext *cc, const char *password) {
    ...
    cc->passwoed_len = strlen(password);
    cc->password = (sds) malloc(cc->passwoed_len +1);
    memcpy(cc->password, password, cc->passwoed_len);
    cc->password[cc->passwoed_len] = 0;
...
int redisClusterAuth(redisClusterContext *cc, redisContext *c) {
    ...
    //AUTH password
    size_t  cmd_len = 4 + 1 + cc->passwoed_len + 1;
    ...
    if (reply == NULL) {
        if (c->err == REDIS_ERR_TIMEOUT) {
            __redisClusterSetError(cc, cc->err, "Command(Auth) reply error(socket timeout)");
        } else {
            __redisClusterSetError(cc, REDIS_ERR_OTHER, "Command(Auth) reply error(NULL).");
        }
        goto error;
        ...
        if (reply->type == REDIS_REPLY_ERROR) {
            ...
            __redisClusterSetError(cc, REDIS_ERR_OTHER, "Command(Auth) reply error: type is not status.");
        }
        goto error;
    }
    if ( 0 != strcmp(reply->str, REDIS_COMMAND_AUTH_OK)) {
        __redisClusterSetError(cc, REDIS_ERR_OTHER, "Command(Auth) reply error: reply-str is not OK.");
        goto error;
    }
    freeReplyObject(reply);
    return REDIS_OK;
